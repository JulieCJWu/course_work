...
class ArbreAVL 
{
  ...
  private:
    ...
    void afficherInordre(Noeud*&); // i added
...
bool ArbreAVL<T>::contient(const T& element) const
{
    ...
    return rechercher(element).courant != NULL;
    return false;
}
...
void ArbreAVL<T>::vider(Noeud*& noeud)
{
    ...
    vider(noeud->droite);
    delete noeud;
    noeud = NULL; // need to set it to null after delete it
}
...
int  ArbreAVL<T>::hauteur() const{
    ...
    while(n!=NULL){
        h++;
        if(n->equilibre==1) // equilibre=hauteurgauche - hauteurdroite
            n=n->gauche;
        else
            n=n->droite;
    }
...
bool ArbreAVL<T>::enlever(Noeud*& noeud, const T& element)
{
    if(element < noeud->contenu)
    {
        ...
        //{
            return (enlever(noeud->gauche, element)); // i added
        //}
    }
    else if(element > noeud->contenu)
    {
        return (enlever(noeud->droite, element)); // i added
    }
    else if(element == noeud->contenu)
    {
...
/*
 // do somethong to mimic this code, and need to update equilibre too
    if(element < noeud->contenu){
        if(inserer(noeud->gauche, element))
        {
            noeud->equilibre++;
...
typename ArbreAVL<T>::Iterateur ArbreAVL<T>::rechercher(const T& e) const
{
    ...
    Noeud* n = racine; 
    while(n){
        if(e < n->contenu){ iter.chemin.empiler(n); n = n->gauche;
        }
        else if(n->contenu < e)
            n = n->droite; 
        else{
...
// fonction publique
template <class T>
void ArbreAVL<T>::afficherInordre(){
 afficherInordre(racine);
 }

// fonction privee
template <class T>
//void ArbreAVL<T>::const afficherInordre(Noeud*& racine)
void ArbreAVL<T>::afficherInordre(Noeud*& racine)
{
  if (racine == NULL) return;
  
  afficherInordre(racine->gauche);
  std::cout << racine->contenu << " ";
  afficherInordre(racine->droite);
